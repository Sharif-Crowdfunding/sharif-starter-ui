...
export default function Banner({ time, project }) {
  ...
  return (
    <Card>
      ...
      <CardHeader>
        <Heading size="lg" fontFamily={"MyShFont"} style={{textAlign: "center"}}>  عنوان پروژه: {project?.name}</Heading>
      </CardHeader>
       <Image src={project?.image} style={{height: "25%", alignSelf:"center"}}/>
      <CardBody>
        <Stack divider={<StackDivider />} spacing="4">
          <Box>
            <Heading size="xm" textTransform="uppercase" fontFamily={"MyShFont"}>
              نام نماد
            </Heading>
